...
def find_duplicate_end_index(start_index, data, seq_len=245760):
    ...
    for i in range(scan - 1):
        if i + seq_len + start_index >= len(
            data
        ):  # this is the case where the end of the duplicate is at the end of the data - instead of finding unequal values, we hit the end of the array.
