...
kd_ref["analysis_root"] = Path(
    "/Volumes/opto_loc/Data/ACHR_PROJECT_MATERIALS/ACHR_3/ACHR_3-analysis-data"
)
...
def load_data(sub_info, exp_list=None, add_time=None):
    ...
    for condition in exp_list:
        if add_time is not None:
            a[condition + "-e-d"], a[condition + "-e-s"] = kd.get_data_spg(
                paths[condition],
                store="EEGr",
                t1=times[condition]["bl_sleep_start"] - 30,
                t2=times[condition]["stim_off"] + add_time,
                channel=[1, 2],
            )
            a[condition + "-f-d"], a[condition + "-f-s"] = kd.get_data_spg(
                paths[condition],
                store="LFP_",
                t1=times[condition]["bl_sleep_start"] - 30,
                t2=times[condition]["stim_off"] + add_time,
...
def acr_rel2peak(spg, hyp, times, band="delta", ylim=None):
    ...
    bp_nrem_rel2peak = bp_nrem_rel2peak.assign_coords(
        time_rel=("datetime", rel_time_index)
    )
