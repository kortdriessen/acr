...
def add_side_col(sub_df, novel=""):
    for cond in sub_df.condition.unique():
        ...
        grid_df = (
            grid_df.groupby(["subject", "condition", "node"])
            .median(numeric_only=True)
            .reset_index()
        )
        midline_x = (
            grid_df.loc[grid_df.node == "midline_upper"]["x"].values[0]
            + grid_df.loc[grid_df.node == "midline_lower"]["x"].values[0]
        ) / 2
        if novel == "left":
...
def create_arena_plot(arena_positions, condition="acq"):
    ...
    midline_x = (
        arena_positions[condition].loc["midline_upper"]["x"]
        + arena_positions[condition].loc["midline_lower"]["x"]
    ) / 2
    upper_arena_y = arena_positions[condition].loc["midline_upper"]["y"]
    lower_arena_y = arena_positions[condition].loc["midline_lower"]["y"]
    left_arena_x = (
        arena_positions[condition].loc["arena_upper_left"]["x"]
        + arena_positions[condition].loc["arena_lower_left"]["x"]
    ) / 2
    right_arena_x = (
        arena_positions[condition].loc["arena_upper_right"]["x"]
        + arena_positions[condition].loc["arena_lower_right"]["x"]
    ) / 2
...
def create_arena_plot_streamlined(ap, buffer_lr=0, buffer_ud=0, return_bounds=False):
    ...
    midline_x = (
        ap.loc[ap["node"] == "midline_upper"]["x"].values[0]
        + ap.loc[ap["node"] == "midline_lower"]["x"].values[0]
    ) / 2
    upper_arena_y = ap.loc[ap["node"] == "midline_upper"]["y"].values[0]
    lower_arena_y = ap.loc[ap["node"] == "midline_lower"]["y"].values[0]
    left_arena_x = (
        ap.loc[ap["node"] == "arena_upper_left"]["x"].values[0]
        + ap.loc[ap["node"] == "arena_lower_left"]["x"].values[0]
    ) / 2
    right_arena_x = (
        ap.loc[ap["node"] == "arena_upper_right"]["x"].values[0]
        + ap.loc[ap["node"] == "arena_lower_right"]["x"].values[0]
    ) / 2
    ...
    if return_bounds:
        return (
            ...
            ax,
            (
                left_arena_x - buffer_lr,
                right_arena_x + buffer_lr,
                upper_arena_y - buffer_ud,
                lower_arena_y + buffer_ud,
            ),
        )
    return f, ax
...
def get_sub_timing(subject):
    sprdsheet_file = (
        "/Volumes/opto_loc/Data/ACR_PROJECT_MATERIALS/NOR_subjects_analysis.xlsx"
    )
...
def load_diffs_csv(subject, smoothing_sigma=None):
    sub_folder = (
        f"/Volumes/neuropixel_archive/Data/acr_archive/NOR_videos/home_boxes/{subject}"
    )
...
def plot_diffs(diffs, box_time, acq_day, smoothing_sigma=None, limit_x=True):
    ...
    ax.axvline(test_start, color="green", linewidth=4)  # ty:ignore[invalid-argument-type]
...
def threshold_actigraph(df, threshold=0.1, col="sm_diff"):
...
def load_subject_node(
    ...
    for cond in conds:
        dat = pd.read_csv(
            f"{data_root}/{subject}-{cond}DLC_Resnet50_nor_single_subJun25shuffle0_snapshot_250_filtered.csv"
        )
        ...
        if check_for_manual_labels(subject, cond) is not False:
            ...
            if total_bad >= min_ex_time:
                print(
                    f"{subject} {cond} has {total_bad} seconds of exclusion, EXCLUDING BAD FRAMES"
                )
...
def threshold_post_dep_sleep(
    subject, smoothing_sigma=2, quantile=0.75, epoc_duration=3600, col_to_use="diff"
):
    ...
    sleep_start_real = dfday.query("datetime > @min_sleep_start")[
        "datetime"
    ].values.min()
    ...
    dfday["state"] = [
        "wake" if x >= threshold else "sleep" for x in dfday[col_to_use].values
    ]

    df_sleep = dfday.loc[
        (dfday["datetime"] >= sleep_start)
        & (dfday["datetime"] < sleep_start + pd.Timedelta(f"{epoc_duration}s"))
    ]
    counts = df_sleep.groupby("state").count().reset_index()
    sleep_proportion = (
        counts.loc[counts["state"] == "sleep"][col_to_use].values[0] / epoc_duration
    )
    ...
    ax.axvspan(
        sleep_start,
        sleep_start + pd.Timedelta(f"{epoc_duration}s"),
        color="orange",
        alpha=0.7,
    )
