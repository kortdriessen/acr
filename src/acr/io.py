...
def get_channels_to_exclude(subject, experiment, which="both", probe=None):
    ...
    if probe == None:
        deads = ex.loc[(ex["subject"] == subject) & (ex["exp"] == experiment)][
            "dead_channels"
        ].values
        all_exs = ex.loc[(ex["subject"] == subject) & (ex["exp"] == experiment)][
            "all_exclude"
        ].values
    elif "NNX" in probe:
        deads = ex.loc[
...
def nuke_bad_chans_from_df(subject, experiment, df, which="dead", probe=None):
    ...
    probe : str
        if this is passed, the channels to exclude will be restricted to the given probe, and only channels on that probe will be excluded.
    """
    chans_to_nuke = get_channels_to_exclude(
        subject, experiment, which=which, probe=probe
    )
    ...
    if type(df) == pd.DataFrame:
        if type(probe) == str:
            if "NNX" in probe:
                return df[
                    ~((df[store_col] == probe) & (df["channel"].isin(chans_to_nuke)))
                ]
        elif probe == None:
        ...
        if type(probe) == str:
            if "NNX" in probe:
                return df.filter(
                    ~(
                        (pl.col(store_col) == probe)
                        & (pl.col("channel").is_in(chans_to_nuke))
                    )
                )
        elif probe == None:
...
def load_hypno_full_exp(subject, exp, corrections=True, float=False, update=True):
    ...
    if float:
        ...
        if corrections == True:
            hypno_final = hu.standard_hypno_corrections(
                hypno_final.reset_index(drop=True)
            )
...
def calc_and_save_bandpower_sets(
    subject,
    stores=["NNXo", "NNXr"],
    recordings=None,
    window_length=4,
    ...
    raise DeprecationWarning(
        "This function is deprecated, use MT_CALC_AND_SAVE_BANDPOWER_SETS instead"
    )
    ...
    for store in stores:
        for recording in recordings:
            ...
            spg = kx.spectral.get_spextrogram(
                data, window_length=window_length, overlap=overlap
            )
...
def MT_calc_and_save_bandpower_sets(
    subject,
    stores=["NNXo", "NNXr"],
    recordings=None,
    seg_length=2,
    ...
    for store in stores:
        for recording in recordings:
            ...
            data = load_raw_data(subject, recording, store)
            spg = kx.spectral.get_mt_spextrogram(
                data,
                subject=subject,
                rec=recording,
...
def _save_bp_set(
    data, subject, recording, store, overwrite=False, folder="mt_bandpower_data"
):
...
def load_raw_data(
    ...
    data = xr.open_dataarray(path, decode_timedelta=False)
    if select:
        data = data.sel(select)
    if np.logical_and(
        recording not in list(data.coords.keys()), store not in list(data.coords.keys())
    ):
...
def load_bandpower_file(
    ...
    if os.path.exists(path) == False:
        ...
        raise FileNotFoundError(
            f"{path} does not exist, run calc_and_save_bandpower_sets(subject, stores=[store], recordings=[recording]) to calculate"
        )
...
def load_concat_bandpower(
    ...
    for store in stores:
        bp_recs = []
        for recording in recordings:
            bp = load_bandpower_file(
                subject,
                recording,
                store,
                hypno=hypno,
                update_hyp=update_hyp,
                exclude_bad_channels=exclude_bad_channels,
                folder=folder,
            )
            bp_recs.append(bp)
...
def load_concat_raw_data(
    ...
    for store in stores:
        data_recs = []
        for recording in recordings:
            data_rec = load_raw_data(
                subject, recording, store, exclude_bad_channels=exclude_bad_channels
            )
...
def _interp_raw_fp_data(data, chans_to_interp, sigma_um=50, p=1.3):
    ...
    assert len(data.channel.values) == 16, (
        "this function is written for 16 channel probes"
    )
    chan_ids = np.arange(1, 17)
    si_rec = si.extractors.NumpyRecording(
        raw_data, sampling_frequency=data.fs, channel_ids=chan_ids
    )
    si_rec = set_probe_and_channel_locations_on_si_rec(si_rec)
    intp_rec = si.preprocessing.interpolate_bad_channels(
        si_rec, chans_to_interp, sigma_um=sigma_um, p=p
    )
...
def interpol_and_save_fp_data(
    ...
    if data is None:
        data = load_raw_data(
            subject, rec, probe, exclude_bad_channels=False, hypno=False
        )
    else:
        data = data
    ...
    if redo == False:
        if (
            acr.info_pipeline.read_interpol_done(subject, rec, probe, version="lfp")
            is not None
        ):
            return
    data = _interp_raw_fp_data(data, chans_to_interp)
    spg = kx.spectral.get_mt_spextrogram(
        data, subject=subject, rec=rec, seg_length=2, overlap=1, NW=4
    )
    bp = kx.spectral.get_bp_set(spg)
    ...
    acr.info_pipeline.write_interpol_done(
        subject, rec, probe, chans=chans_to_interp, version="lfp"
    )
    return


def interpolate_exp_lfp_data(
    subject, exp, probes=["NNXo", "NNXr"], redo=False, folder="mt_bandpower_data"
):
    ...
    for rec in recs_to_interp:
        for probe in probes:
            if redo == False:
                if (
                    acr.info_pipeline.read_interpol_done(
                        subject, rec, probe, version="lfp"
                    )
                    is not None
                ):
                    continue
...
def read_full_df(folder="reb-bp__1h-cum__mean-rel__full-bl", subs_exp=True):
    ...
    if subs_exp == False:
        ...
        parquet_files = [
            f for f in os.listdir(data_directory) if f.endswith(".parquet")
        ]
        ...
        for subject in swi_subs_exps.keys():
            for exp in swi_subs_exps[subject]:
                df = read_subject_rebdf(
                    subject, exp, folder=folder, exclude_bad_chans=True
                )
                dataframes.append(df)
        reb_df = pd.concat(dataframes, ignore_index=True)
...
def read_subject_rebdf(
    sub, exp, folder="reb-bp__1h-cum__mean-rel__full-bl", exclude_bad_chans=True
):
