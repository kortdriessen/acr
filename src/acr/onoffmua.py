...
def label_oodf_with_hypno_conditions(oodf, hd):
    ...
    for key in hd.keys():
        for bout in hd[key].itertuples():
            oodf_pd.loc[
                (
                    (oodf_pd["start_datetime"] >= bout.start_time)
                    & (oodf_pd["end_datetime"] <= bout.end_time)
                ),
                "condition",
            ] = key
...
def make_oodf_relative(
    oodf, by="channel", ref_to="early_bl", avg_method="mean", make_rel_col="duration"
):
    print(
        "acr.oo_utils.relativize_oodf is better (unless operating on single channels!), use that where possible!"
    )
    ...
    if by == "channel":
        ref_avgs = (
            refdf.groupby(["probe", "channel", "status"]).median()
            if avg_method == "median"
            else refdf.groupby(["probe", "channel", "status"]).mean()
        )
        ...
        for probe in oodf["probe"].unique():
            for channel in oodf["channel"].unique():
                for status in oodf["status"].unique():
                    bl_val = (
                        ref_avgs.filter(pl.col("probe") == probe)
                        .filter(pl.col("channel") == channel)
                        .filter(pl.col("status") == status)[make_rel_col]
                        .to_numpy()[0]
                    )
        ...
        ref_avgs = (
            refdf.groupby(["probe", "status"]).median()
            if avg_method == "median"
            else refdf.groupby(["probe", "status"]).mean()
        )
        oodf = oodf.with_columns(pl.lit(101).alias(f"rel_{make_rel_col}"))
        for probe in oodf["probe"].unique():
            for status in oodf["status"].unique():
                ...
                oodf = oodf.with_columns(
                    pl.when(((pl.col("probe") == probe) & (pl.col("status") == status)))
                    .then(pl.col(make_rel_col) / bl_val)
                    .otherwise(pl.col(f"rel_{make_rel_col}"))
                    .alias(f"rel_{make_rel_col}")
                )
...
def _oodf_freqs_by_chan(oodf, hd):
    ...
    for freq_cond in hd.keys():
        ...
        for bout in cdn_hyp.itertuples():
            ...
            counts = (
                condoo.oots(bout.start_time, bout.end_time)
                .groupby(["probe", "channel", "status"])
                .agg(pl.count("duration").alias("pure_count"))
            )
    ...
    for probe in total_freqs["probe"].unique():
        for channel in total_freqs["channel"].unique():
            for status in total_freqs["status"].unique():
                bl_val = (
                    bl_freqs.filter(pl.col("probe") == probe)
                    .filter(pl.col("channel") == channel)
                    .filter(pl.col("status") == status)["freq"]
                    .to_numpy()[0]
                )
                total_freqs = total_freqs.with_columns(
                    pl.when(
                        (
                            (pl.col("probe") == probe)
                            & (pl.col("channel") == channel)
...
def compute_oodf_from_t_array(t, off_min_dur=0.050000, synth_dur=2.0):
    ...
    if type(msk) == np.bool_:
        offs = pd.DataFrame(
            {"start_datetime": t[0], "end_datetime": t[0], "status": "off"}, index=[0]
        )
        ons = pd.DataFrame(
            {"start_datetime": t[0], "end_datetime": t[0], "status": "on"}, index=[0]
        )
        oodf = pd.concat([ons, offs])
        oodf["duration"] = (
            oodf["end_datetime"] - oodf["start_datetime"]
        ).dt.total_seconds()
        return oodf
    ...
    if msk[0]:
        start_indices = np.r_[
            0, start_indices
        ]  # Add the first index if it starts with True
    if msk[-1]:
        end_indices = np.r_[
            end_indices, len(msk)
        ]  # Add the last index if it ends with True

    valid_lengths = (end_indices - start_indices) >= 2
    all_lengths = end_indices - start_indices
    print(
        f"Found {len(all_lengths)} off periods, {sum(valid_lengths)} of which are valid"
    )
    ...
    off_det["duration"] = (
        off_det["end_datetime"] - off_det["start_datetime"]
    ).dt.total_seconds()
    ...
    on_det["duration"] = (
        on_det["end_datetime"] - on_det["start_datetime"]
    ).dt.total_seconds()
    oodf = (
...
def _get_full_oodf_by_channel_slow(mua, off_min_dur=0.050, synth_dur=2.0):
    oodfs = []
    ...
    for probe in mua["probe"].unique():
        for rec in mua.prb(probe)["recording"].unique():
            for chnl in mua.prb(probe)["channel"].unique():
                rec_mua = mua.filter(
                    (pl.col("recording") == rec)
                    & (pl.col("channel") == chnl)
                    & (pl.col("probe") == probe)
                )
                t = rec_mua["datetime"].to_pandas()
                oodf = compute_oodf_from_t_array(
                    t, off_min_dur=off_min_dur, synth_dur=synth_dur
                )
...
def compute_full_oodf_by_channel(mua, off_min_dur=0.050, synth_dur=2.0):
    oodfs = []
    subject = mua["subject"].unique()[0]
    chan_times = (
        mua.group_by(["recording", "probe", "channel"])
        .agg(pl.col("datetime"))
        .sort(["recording", "probe", "channel"])
    )
    ...
    channels.sort()
    assert len(chan_times) == len(recs) * len(probes) * len(channels), (
        "This function relies on an exact match of recording, probe, and channel across the mua dataframe. All recordings must have both probes, both of which must have all channels. Use the _get_full_oodf_by_channel_slow function if this is not the case."
    )
...
def compute_full_oodf_by_probe(mua, off_min_dur=0.050, synth_dur=2.0):
    oodfs = []
    subject = mua["subject"].unique()[0]
    for probe in mua["probe"].unique():
        for rec in mua.prb(probe)["recording"].unique():
            rec_mua = mua.filter(
                (pl.col("recording") == rec) & (pl.col("probe") == probe)
            ).sort("datetime")
            t = rec_mua["datetime"].to_pandas()
            oodf = compute_oodf_from_t_array(
                t, off_min_dur=off_min_dur, synth_dur=synth_dur
...
def true_strictify_oodf(oodf, min_on=0.015):
    oodf = oodf.with_columns(
        [
    ...
    oodf = oodf.with_columns(
        pl.when(
            (pl.col("prev_duration") <= min_on)
            & (pl.col("next_duration") <= min_on)
            & (pl.col("status") == "off")
        )
        .then(pl.lit("on"))
...
def _gen_oodf_mask(oodf):
    ...
    for probe in arrs.keys():
        full_mask[probe] = xr.DataArray(
            arrs[probe],
            dims=["channel", "datetime"],
            coords={"channel": range(1, 17), "datetime": mask_times},
        )
    return full_mask
...
def gen_oodf_mask(subject, exp, save=False):
    ...
    oodf = compute_full_oodf_by_channel(mua, off_min_dur=0.05, synth_dur=2.50)
    oodf = true_strictify_oodf(oodf, min_on=0.020)
    mask, mask_times = mask_full_oodf(oodf)
    arrs = {}
    for probe in mask.keys():
        arr = np.array(list(mask[probe].values()))
...
def make_oofreqs_relative(freqs, by="channel", ref_to="early_bl"):
    ...
    if by == "channel":
        ref_avgs = refdf.group_by(["probe", "channel", "status"]).agg(
            (pl.sum("pure_count") / pl.sum("bout_duration")).alias("freq_total")
        )
            ...
            for channel in freqs["channel"].unique():
                for status in freqs["status"].unique():
                    bl_val = (
                        ref_avgs.filter(pl.col("probe") == probe)
                        .filter(pl.col("channel") == channel)
                        .filter(pl.col("status") == status)["freq_total"]
                        .to_numpy()[0]
                    )
        ...
        ref_avgs = refdf.group_by(["probe", "status"]).agg(
            (pl.sum("pure_count") / pl.sum("bout_duration")).alias("freq_total")
        )
        freqs = freqs.with_columns(rel_freq=pl.lit(101))
        for probe in freqs["probe"].unique():
...
def make_oofreqs_relative_bouts(
    freqs, by="channel", ref_to="early_bl", avg_method="mean"
):
    if type(freqs) != pl.DataFrame:
        freqs = pl.DataFrame(freqs)
    assert "condition" in freqs.columns, "freqs must be labeled with condition"
...
def bin_spikes(
    ...
    return xr.DataArray(
        binned, dims=("channel", "datetime"), coords={"datetime": time_index}
    )


def calculate_off_df_on_spike_bins(
    bin_counts, subject, exp, probe, chan_threshold=15, n_consecutive_bins=50
):
    """
    ...
    start_indices, lengths = find_consecutive_runs(
        off_counts, threshold=chan_threshold, min_length=n_consecutive_bins
    )
