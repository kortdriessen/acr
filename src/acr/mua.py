...
def prepro_np_array_via_si(np_array, fs):
    ...
    si_filt_cmr_with_probe = kde.units.utils.set_probe_and_channel_locations_on_si_rec(
        si_filt_cmr
    )
...
def save_preprocessed_mua_data(
    data,
    subject,
    recording,
    probe,
        ...
        for rec, dur in zip(recs, durations):
            ...
            if exists == True:
                if overwrite == True:
                    ...
                    print(
                        f"Preprocessed data already exists for {subject}--{rec}--{probe}, set overwrite=True to overwrite"
                    )
                    continue
            else:
            ...
            if interpol == True:
                ...
                preprocessed_data = _interp_mua_data(
                    preprocessed_data, chans_to_interp, sigma_um=50, p=1.3
                )
            save_preprocessed_mua_data(
                preprocessed_data, subject, rec, probe, overwrite=overwrite, njobs=njobs
            )
            if interpol == True:
                acr.info_pipeline.write_interpol_done(
                    subject, rec, probe, chans=chans_to_interp, version="ap"
                )
    return
...
def load_processed_mua_signal(subject, recording, probe, version="xr"):
    path = (
        f"{raw_data_root}mua_data/{subject}/MUA--{subject}--{recording}--{probe}.zarr"
    )
    if version == "xr":
...
def detect_mua_spikes_si(
    ...
    if raw_mua_exists == False:
        print(
            f"Preprocessed data does not exist for {subject}--{recording}--{probe}, run preprocess_data_for_mua first"
        )
        return None
    if (overwrite == False) and (save == True):
        if check_for_mua_spikes_df(subject, recording, probe):
            print(
                f"Peaks already detected for {subject}--{recording}--{probe}, set overwrite=True to overwrite"
            )
            return None
...
def detect_all_subject_mua_spikes(
    subject,
    threshold=4,
    chunk_duration="1s",
    n_jobs=224,
...
def load_detected_mua_spikes(subject, rec, probe, dt=True):
    ...
    df = df.with_columns(
        channel=pl.col("channel") + 1
    )  # because initially the channel index was zero-based.
    if dt == True:
...
def load_mua_full_exp(subject, exp, probes=["NNXo", "NNXr"], exclude_dead_chans=True):
    ...
    if exclude_dead_chans:
        ...
        return acr.io.nuke_bad_chans_from_df(
            subject, exp, full_df, which="dead", probe=None
        )
    else:
        return full_df
...
def detect_peaks_on_full_concat_recording(
    subject, exp, njobs=112, probes=["NNXr", "NNXo"], overwrite=False
):
    for probe in probes:
        if _check_concat_df_exists(subject, exp, probe) and overwrite == False:
            print(
                f"Peaks already detected for {subject}--{exp}--{probe}, set overwrite=True to overwrite"
            )
            continue
        si_recs = []
        sort_id = f"{exp}-{probe}"
        ...
        combo_noise = rt.get_noise_levels(
            fullsi,
            random_slices_kwargs={
                "num_chunks_per_segment": int(len(recs) * 100),
                "chunk_size": int(fullsi.get_sampling_frequency() * 2),
            },
        )
        ...
        job_kwargs = dict(chunk_duration="100s", n_jobs=njobs, progress_bar=True)
...
def full_mua_pipeline_for_subject(
    subject,
    list_of_exps=None,
    df_version="concat",
    interpol=False,
    ...
    for exp in list_of_exps:
        ...
        preprocess_data_for_mua(
            subject, exp, overwrite=overwrite, interpol=interpol, njobs=32
        )

        # 2. Run the detection on the preprocessed data, either by recording, or on the concatenated data, or both
        if df_version == "concat":
            detect_peaks_on_full_concat_recording(
                subject, exp, njobs=detect_jobs, overwrite=overwrite
            )
...
def interpol_and_resave_mua_data(subject, exp, probes=["NNXo", "NNXr"], redo=False):
    for probe in probes:
        ...
        for rec in recs:
            assert check_for_preprocessed_mua_data(subject, rec, probe), (
                f"no preprocessed data found for {subject}--{rec}--{probe}"
            )
            interped_chans = acr.info_pipeline.read_interpol_done(
                subject, rec, probe, version="ap"
            )
            if interped_chans is not None:
                if redo == False:
                    print(
                        f"interpolated data already exists for {subject}--{rec}--{probe}, set redo=True to interpolate again, but do this with caution!"
                    )
                    continue
            chans_to_interp = acr.info_pipeline.get_interpol_info(subject, probe)
            if (
                len(chans_to_interp) == 0
            ):  # if there is nothing to interpolate for this probe, we just continue
                continue
            ...
            acr.info_pipeline.write_interpol_done(
                subject, rec, probe, chans=interped_chans, version="ap"
            )
    return
