...
def format_rec_quality(path):
    ...
    worksheet.conditional_format(
        "F1:F1000",
        {"type": "text", "criteria": "containing", "value": "No", "format": format_red},
    )

    # Do the durations match?
    worksheet.conditional_format(
        "G2:G1000", {"type": "cell", "criteria": ">", "value": 0, "format": format_red}
    )
...
stores = st.sidebar.multiselect(
    "Stores for LFP/Bandpower data: ",
    ["NNXr", "NNXo", "EEGr", "EMGr"],
    ["NNXr", "NNXo", "EEGr", "EMGr"],
)

st.write(
    "Make sure before proceeding that the following two things are completely updated: "
)
...
st.write(
    "Next we update the end_times.yaml to check for any weird endings in the recordings that need to be cut off (because of amplifier problems, etc.)"
)
if st.button("Update end_times.yaml"):
    ...
    important_recs = yaml.safe_load(
        open("/Volumes/opto_loc/Data/ACR_PROJECT_MATERIALS/important_recs.yaml", "r")
    )
    end_info = yaml.safe_load(
        open("/Volumes/opto_loc/Data/ACR_PROJECT_MATERIALS/end_times.yaml", "r")
    )
        ...
        for exp in important_recs[sub]:
            ...
            for rec in important_recs[sub][exp]:
                for store in stores:
                    ...
                    data = tdt.read_block(
                        si["paths"][rec], store=store, channel=14, t1=0, t2=0
                    )
                    ...
                    if zero_period_start == [0]:
                        ...
                        st.write(
                            f"Found a zero ending for {rec} - {store} | location: {zero_period_start}"
                        )
                    with open(
                        "/Volumes/opto_loc/Data/ACR_PROJECT_MATERIALS/end_times.yaml",
                        "w",
                    ) as f:
...
st.write(
    "Next we update the duplication_info.yaml to check for any duplicated sequences spaced by 245760 samples"
)
st.write(
    "Before Proceeding, make sure that the end_times.yaml file is completely updated (and has been checked after running the above)"
)
        ...
        for exp in important_recs[sub]:
            ...
            for rec in important_recs[sub][exp]:
                for store in stores:
                    ...
                    data = tdt.read_block(
                        si["paths"][rec], store=store, channel=14, t1=0, t2=t2
                    )
                    data = data.streams[store].data
                    ...
                    for i in range(len(starts)):
                        f, ax, dup_len = dpf.plot_duplicate(
                    ...
                    with open(
                        "/Volumes/opto_loc/Data/ACR_PROJECT_MATERIALS/duplication_info.yaml",
                        "w",
                    ) as f:
...
st.write(
    "Finally we update the master_rec_quality.xlsx to aggregate all of this information"
)
if st.button("Update master_rec_quality.xlsx"):
    ...
    dic_list = []
    for sub in important_recs.keys():
        if sub != subject:  # we only update the current subject
            continue
        si = acr.info_pipeline.load_subject_info(sub)
        stores = important_recs[sub]["stores"]
        for exp in important_recs[sub]:
    ...
    new_rec_quality = (
        pd.concat([rec_quality, df], ignore_index=True) if not df.empty else rec_quality
    )
                ...
                for store in important_recs[sub]["stores"]:
                    if "NNXr" and "NNXo" in important_recs[sub]["stores"]:
                        ...
                        diff = (
                            ...
                            - si["rec_times"][rec][f"{other}-duration"]
                        )
                    else:
                        diff = 0
                    ...
                    ix = (
                        ...
                        .loc[new_rec_quality.store == store]
                        .index.values[0]
                    )
...
st.write(
    "Here we can generate the downsampled LFP data, and then once that is done, the bandpower data for this subject"
)

if st.button("Process LFPs (downsample and save raw data)"):
