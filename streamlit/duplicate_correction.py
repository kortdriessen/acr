...
end_chunk_removal = st.sidebar.multiselect('Choose recordings and stores to remove end chunks from', end_chunk_combos, None)
    ...
    for rec in recs:
        for store in stores:
            ...
            for channel in channels:
                ...
                for fl in os.listdir(path): # this also gets us the requiste .sev file names for when we write data
                ...
                if len(nonst_dups) >=1:
                    for i, dup_pos in enumerate(nonst_dups):
                        ...
                        if dpf._confirm_dup_position(start, end, data) == False: # if the duplicate position is not confirmed, we need to find it (since we only log the duplicate position for one channel, almost always we will have to use this)
                            start, end = dpf.starts_and_ends_nonst(data)
                            start = start[0] # can do this because we should always be going after the first non-standard duplicate
                            end = end[0]
                            assert start-di['starts'][dup_pos] <= 10 # these are just to ensure that we are taking out the correct duplicate
                    ...
                    st.write('after non-standard duplicates removed, length of data is: ', len(data))
                ...
                if len(st_dups) >= 1:
                    for i, dup_pos in enumerate(st_dups):
                        ...
                        if dpf._confirm_dup_position(start, end, data) == False and i==0:
                            print(f'indexes recorded in dup_info did not match, finding actual coordinates now')
                        ...
                        if i > 0 and len(corrected_data) > 1000:
                            print(f'standard duplicate #{i}, need to find duplicate again as original coordinates are no longer valid, will use corrected data')
                            start, end = dpf.starts_and_ends_standard(corrected_data)
                            start = start[0]
                            end = end[0] 
                        
                        if i>0:
                            assert dpf._confirm_dup_position(start, end, corrected_data) == True
                            slice1 = corrected_data[:start+seq_len]
                            slice2 = corrected_data[end+seq_len+1:]
                            corrected_data = np.concatenate([slice1, slice2])
                    assert len(corrected_data) == len(data) - len(st_dups)*seq_len #ensures that we end with data of the correct length
                ...
                if f'{rec}-{store}' in end_chunk_removal:
                    st.write(f'Length of data before end chunk removal: {len(corrected_data)}')
                ...
                st.write(f'Duplicate correction complete for channel {channel}, writing corrected data || {subject}, {rec}, {store}')
                ...
                st.write(f'Length of corrected data just before writing to rawsev[store]: {len(corrected_data)}')
                rawsev[store].data = corrected_data 

                # some more stuff from mark hanus (update the new file size in the header)
                st.write(f'Length of corrected data just before writing to header: {rawsev[store].data.size}')
                new_file_size = np.uint64(rawsev[store].data.size * rawsev[store].data.itemsize + 40)
...
st.markdown('This removes each of the recordings already selected from all fields of subject_info.yaml, and then runs update_subject_info()')
